...
function isUserExist($userId){
    $CI = &get_instance();
    $CI->db->where('restaurant_id',$userId);
    $result = $CI->db->get(db_prefix().'restaurant_registration')->result_array();  
...
function get_menu_categories()
{
    ...
    return $CI->db->get(db_prefix() . 'menu_categories')->result_array();
}
...
function get_months()
{
    ...
    $result = $CI->db->get(db_prefix() . 'month')->result_array();
    return $result;
}
...
{
    $CI = &get_instance();
    $CI->db->where('validated', 1);
    $result = $CI->db->get(db_prefix() . 'restaurant_menu_categories')->result_array();
    ...
    for ($i = 0; $i < count($categories); $i++) {
        ...
        $CI->db->where('validated', 1);
        $CI->db->where_in('category', $categories[$i]);
        $result = $CI->db->get(db_prefix() . 'restaurant_menu_categories')->result_array();
        $category[$i] = $result[0];
    }
    return $category;
...
function _maybe_user_is_trying_to_login_into_the_clients_area_as_staff($data)
{
    ...
    if (
        total_rows('staff', ['email' => $data['email']]) > 0
